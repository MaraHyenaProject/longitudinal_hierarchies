################################################################################
#              Process, model, plot empirical data - females                   #
#                                                                              #
#                                                                              #
#                            By Eli Strauss                                    #
#                                                                              #
...
##Compare methods - females

##Read in aggression data
aggression <- read.csv(file = '0.rawdata/tblAggression.csv')
female.aggs <- filter(aggression, aggressor %in% female.ranks$id, recip %in% female.ranks$id)
female.aggs$year <- format(as.Date(female.aggs$date), '%Y')
...
current_row <- 1
periods <- unique(female.ranks$period)
periods <- periods[2:(length(periods)-1)]
for(current.period in periods){
  for(current.method in methods){
    franks_method <- filter(female.ranks, method == current.method)
    ...
    rank_diffs <- left_join(filter(female.aggs, year == current.period, aggressor %in% ids_this_year, recip %in% ids_this_year), filter(franks_method, period == current.period), by = c('recip' = 'id'))$rank -
      left_join(filter(female.aggs, year == current.period, aggressor %in% ids_this_year, recip %in% ids_this_year), filter(franks_method, period == current.period), by = c('aggressor' = 'id'))$rank
    ...
    rank_diffs <- left_join(filter(female.aggs, year == current.period, aggressor %in% ids_last_year, recip %in% ids_last_year), filter(franks_method, period == (current.period-1)), by = c('recip' = 'id'))$rank -
      left_join(filter(female.aggs, year == current.period, aggressor %in% ids_last_year, recip %in% ids_last_year), filter(franks_method, period == (current.period-1)), by = c('aggressor' = 'id'))$rank
...
summary(glm.out)
pcomp_table(mod = glht(glm.out, mcp(Method = 'Tukey')), title ='prop_consistent_this_year')
...
summary(glm.out)
pcomp_table(glht(glm.out, mcp(Method = 'Tukey')), title = 'prop_consistent_next_year')
...
summary(glm.out)
pcomp_table(glht(glm.out, mcp(method = 'Tukey')), 'delta_active')

glm.out <- lme4::glmer(data = model_changes, delta.passive ~ method + (1|id), family = "poisson")
summary(glm.out)
pcomp_table(glht(glm.out, mcp(method = 'Tukey')), 'delta_passive')
...
##Figure 7
##plot a line for each female for Informed MatReorder ranks 
p1 <- ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'informed_matreorder'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
p2 <- ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'isi'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
p3 <-ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'informed_elo'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
p4 <-ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'elo'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
p5 <-ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'informed_ds'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
p6 <-ggplot(data = filter(female.ranks, period >= 1988, period <= 2013, method == 'ds'), aes(y = rank, x = period)) + 
  ylim(55,0)+
  theme_classic() + 
  xlab('Period')+
  ylab('Rank')+
...
pdf(file = 'plots/8.Fig5.pdf', 8, 11)
...
cairo_pdf(file = 'plots/8.Fig6.pdf', 5,3)
